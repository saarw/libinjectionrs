...
impl TokenizerDebugger {
    ...
    fn analyze_characters(&self, input: &[u8]) -> Result<Vec<CharacterAnalysis>, Box<dyn std::error::Error>> {
        ...
        for (pos, &byte) in input.iter().enumerate() {
            ...
            // Get actual character type from the autogenerated mapping  
            let char_type = libinjectionrs::sqli::sqli_data::get_char_type(byte);
            let char_type_str = format!("{:?}", char_type);
            ...
            analysis.push(CharacterAnalysis {
                ...
                parser_function: "from_char_type".to_string(), // Will be determined by tokenizer dispatch
            });
    ...
    fn analyze_c_tokenization(&self, input: &[u8]) -> Result<CResults, Box<dyn std::error::Error>> {
        ...
        let harness_path = "libinjection-debug/c_harness/debug_harness";
